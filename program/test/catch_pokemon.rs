...
mod catch_pokemon {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("nft_pokemon", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let element: String = Default::default();
		...
		let (pokemon_pubkey, _) = Pubkey::find_program_address(
			&[b"pokemon", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_info.pubkey(), true),
			AccountMeta::new(pokemon_pubkey, false),
		...
		let data = NftPokemonInstruction::CatchPokemon(crate::generated::instructions::CatchPokemonArgs{
			element,
			rarity,
			short_description,
		});
